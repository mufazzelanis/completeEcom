    ...
    <form action="{{ route('admin.batches.store') }}" method="POST">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    ...
                    <select name="product_id" required
                        ...
                        <option value="{{ $p->id }}" {{ old('product_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}{{ $p->sku ? ' ('.$p->sku.')' : '' }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="flex-1 bg-indigo-600 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition">Add Batch</button>
