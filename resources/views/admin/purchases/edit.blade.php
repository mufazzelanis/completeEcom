...
@section('title', 'Edit Purchase — '.$purchase->reference_no)

@section('content')
<div class="max-w-5xl">
    <a href="{{ route('admin.purchases.show', $purchase) }}" class="text-indigo-600 hover:text-indigo-700 text-sm flex items-center space-x-2 mb-6">
    ...
    <form action="{{ route('admin.purchases.update', $purchase) }}" method="POST"
        x-data="{
            items: {{ Js::from($purchase->items->map(fn($i) => ['product_id'=>$i->product_id,'quantity_ordered'=>$i->quantity_ordered,'unit_cost'=>$i->unit_cost])->values()) }},
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div class="grid grid-cols-12 gap-2 text-xs text-gray-500 font-medium px-1 mb-2">
                        <div class="col-span-5">Product</div><div class="col-span-2">Qty</div>
                        <div class="col-span-3">Unit Cost (৳)</div><div class="col-span-1 text-right">Total</div><div class="col-span-1"></div>
                    </div>
                    <template x-for="(item, i) in items" :key="i">
                        <div class="grid grid-cols-12 gap-2 items-center mb-2">
                            <div class="col-span-5">
                                <select :name="`items[${i}][product_id]`" x-model="item.product_id"
                                    ...
                                    <template x-for="p in products" :key="p.id">
                                        <option :value="p.id" :selected="p.id == item.product_id" x-text="p.name + (p.sku ? ' ('+p.sku+')' : '')"></option>
                                    </template>
                                </select>
                            </div>
                            ...
                            <div class="col-span-3">
                                <input type="number" :name="`items[${i}][unit_cost]`" x-model="item.unit_cost" min="0" step="0.01"
                                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <textarea name="notes" rows="3" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $purchase->notes) }}</textarea>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" value="{{ $purchase->reference_no }}" readonly
                                class="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-2.5 text-sm font-mono text-gray-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Supplier *</label>
                            <select name="supplier_id" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <input type="date" name="purchased_at" value="{{ old('purchased_at', $purchase->purchased_at->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                @foreach(['draft','ordered','partial','cancelled'] as $st)
                                <option value="{{ $st }}" {{ old('status', $purchase->status) == $st ? 'selected' : '' }}>{{ ucfirst($st) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Amount Paid (৳)</label>
                            <input type="number" name="paid_amount" value="{{ old('paid_amount', $purchase->paid_amount) }}" min="0" step="0.01"
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        </div>
                    </div>
                </div>
                <button type="submit" class="w-full bg-indigo-600 text-white py-3 rounded-xl font-semibold hover:bg-indigo-700 transition">Update Purchase</button>
            </div>
        </div>
    </form>
</div>
