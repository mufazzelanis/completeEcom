...
@section('content')
<div class="max-w-xl">
    <a href="{{ route('admin.reviews.index') }}" class="text-indigo-600 hover:text-indigo-700 text-sm flex items-center gap-2 mb-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <div class="flex items-center gap-3 mb-5 pb-5 border-b border-gray-100">
            ...
            <img src="{{ \Illuminate\Support\Facades\Storage::url($review->product->image) }}" class="w-14 h-14 object-cover rounded-xl border border-gray-100">
            @endif
            <div>
                <p class="font-semibold text-gray-800">{{ $review->product->name ?? 'Unknown product' }}</p>
                <p class="text-xs text-gray-400">by {{ $review->user->name ?? 'Unknown customer' }} &middot; {{ $review->created_at->format('d M Y') }}</p>
            </div>
        </div>

        <form action="{{ route('admin.reviews.update', $review) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="flex items-center space-x-3">
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_approved" value="1" {{ old('is_approved', $review->is_approved) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.reviews.index') }}" class="bg-gray-100 text-gray-600 px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-200 transition">Cancel</a>
            </div>
        </form>
    </div>
</div>
