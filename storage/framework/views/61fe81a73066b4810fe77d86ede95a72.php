...
<?php if($errors->any()): ?><div class="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm"><?php echo e($errors->first()); ?></div><?php endif; ?>

<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">

    <div class="bg-white rounded-2xl shadow-sm p-6">
        <h3 class="font-semibold text-gray-800 mb-4">Add Tag</h3>
        <form action="<?php echo e(route('admin.tags.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <input type="text" name="name" value="<?php echo e(old('name')); ?>" placeholder="Tag name…"
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
            <form method="GET" class="flex gap-3">
                ...
                <?php if(request('search')): ?><a href="<?php echo e(route('admin.tags.index')); ?>" class="px-4 py-2 border border-gray-200 rounded-xl text-sm text-gray-600">Clear</a><?php endif; ?>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50" x-data="{ editing: false, name: '<?php echo e($tag->name); ?>' }">
                        <td class="px-6 py-3">
                            <div x-show="!editing" class="text-sm font-medium text-gray-800"><?php echo e($tag->name); ?></div>
                            <form x-show="editing" x-cloak action="<?php echo e(route('admin.tags.update', $tag)); ?>" method="POST" class="flex items-center gap-2">
                        ...
                        <td class="px-6 py-3 text-xs font-mono text-gray-400"><?php echo e($tag->slug); ?></td>
                        <td class="px-6 py-3 text-center text-sm text-gray-600"><?php echo e($tag->products_count); ?></td>
                        <td class="px-6 py-3 text-right flex items-center justify-end gap-3">
                            ...
                            <form action="<?php echo e(route('admin.tags.destroy', $tag)); ?>" method="POST" onsubmit="return confirm('Delete tag?')">
            ...
            <?php if($tags->hasPages()): ?><div class="px-6 py-4 border-t border-gray-100"><?php echo e($tags->links()); ?></div><?php endif; ?>
        </div>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\completeEcom\resources\views/admin/tags/index.blade.php ENDPATH**/ ?>