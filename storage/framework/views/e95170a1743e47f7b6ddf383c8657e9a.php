...
<div class="max-w-5xl">
    ...
    <div class="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
        <ul class="list-disc list-inside space-y-1"><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li class="text-sm text-red-600"><?php echo e($e); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul>
    </div>
    <?php endif; ?>

    <form action="<?php echo e(route('admin.products.update', $product)); ?>" method="POST" enctype="multipart/form-data"
        x-data="{
            productType: '<?php echo e(old('type', $product->type ?? 'simple')); ?>',
            variants: <?php echo e(Js::from($product->variants->toArray())); ?>,
            colors: <?php echo e(Js::from($product->colors->toArray())); ?>,
            bundleItems: <?php echo e(Js::from($product->bundleItems->map(fn($b) => ['product_id' => $b->item_product_id, 'quantity' => $b->quantity, 'discount_pct' => $b->discount_pct])->toArray())); ?>,
            faqs: <?php echo e(Js::from($product->faqs->map(fn($f) => ['question' => $f->question, 'answer' => $f->answer])->toArray())); ?>,
            specs: <?php echo e(Js::from($product->specs->map(fn($s) => ['key' => $s->spec_key, 'value' => $s->spec_value])->toArray())); ?>,
            selectedTags: <?php echo e(Js::from($product->tags->map(fn($t) => ['id' => $t->id, 'name' => $t->name])->toArray())); ?>,
            tagQuery: '',
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
            ...
            <div class="grid grid-cols-4 gap-3">
                ...
                <label class="flex flex-col items-center p-4 border-2 rounded-xl cursor-pointer transition"
                    ...
                    <svg class="w-5 h-5 mb-1" :class="productType==='<?php echo e($val); ?>' ? 'text-indigo-600' : 'text-gray-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($icon); ?>"/></svg>
                    <span class="text-sm font-semibold" :class="productType==='<?php echo e($val); ?>' ? 'text-indigo-700' : 'text-gray-700'"><?php echo e($label); ?></span>
                    <span class="text-xs text-gray-400 text-center mt-0.5"><?php echo e($desc); ?></span>
                </label>
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="short_description" rows="2"
                                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><?php echo e(old('short_description', $product->short_description)); ?></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Full Description</label>
                            <textarea name="description" rows="6"
                                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><?php echo e(old('description', $product->description)); ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div class="mb-3"><p class="text-xs text-gray-500 mb-1">Current main image</p>
                        <img src="<?php echo e(Storage::url($product->image)); ?>" class="w-24 h-24 object-cover rounded-xl border border-gray-100"></div>
                    <?php endif; ?>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1"><?php echo e($product->image ? 'Replace Main Image' : 'Main Image'); ?></label>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            <label class="relative cursor-pointer group">
                                <input type="checkbox" name="delete_images[]" value="<?php echo e($img->id); ?>" class="absolute top-1 right-1 w-4 h-4 accent-red-500">
                                <img src="<?php echo e(Storage::url($img->image)); ?>" class="w-16 h-16 object-cover rounded-lg group-hover:opacity-75 transition">
                            </label>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6" x-show="productType === 'digital'" x-cloak>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-xl p-3 text-sm text-green-700">
                            Current file: <span class="font-mono"><?php echo e(basename($product->download_file)); ?></span>
                        </div>
                        <?php endif; ?>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1"><?php echo e($product->download_file ? 'Replace File' : 'Upload File *'); ?></label>
                        ...
                        <div>
                            ...
                            <input type="number" name="download_expiry_days" value="<?php echo e(old('download_expiry_days', $product->download_expiry_days)); ?>" min="1" placeholder="Blank = no limit"
                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6"
                    x-data="{
                        ...
                        categoryId: '<?php echo e(old('category_id', $product->category_id)); ?>',
                        subcategoryId: '<?php echo e(old('subcategory_id', $product->subcategory_id)); ?>',
                ...
                <div class="bg-gray-50 rounded-2xl p-4 text-xs text-gray-500 space-y-1.5">
                    <div class="flex justify-between"><span>Slug</span><span class="font-mono text-gray-600 truncate max-w-32"><?php echo e($product->slug); ?></span></div>
                    <div class="flex justify-between"><span>Variants</span><span class="font-semibold text-gray-700"><?php echo e($product->variants->count()); ?></span></div>
                    <div class="flex justify-between"><span>Colors</span><span class="font-semibold text-gray-700"><?php echo e($product->colors->count()); ?></span></div>
                    <div class="flex justify-between"><span>Gallery</span><span class="font-semibold text-gray-700"><?php echo e($product->images->count()); ?></span></div>
                    <div class="flex justify-between"><span>FAQs</span><span class="font-semibold text-gray-700"><?php echo e($product->faqs->count()); ?></span></div>
                    <div class="flex justify-between"><span>Specs</span><span class="font-semibold text-gray-700"><?php echo e($product->specs->count()); ?></span></div>
                    <div class="flex justify-between"><span>Tags</span><span class="font-semibold text-gray-700"><?php echo e($product->tags->count()); ?></span></div>
                    <div class="flex justify-between"><span>Views</span><span class="font-semibold text-gray-700"><?php echo e(number_format($product->views)); ?></span></div>
                </div>
        ...
        <?php echo $__env->make('admin.products._seo_fields', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    </form>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\completeEcom\resources\views/admin/products/edit.blade.php ENDPATH**/ ?>