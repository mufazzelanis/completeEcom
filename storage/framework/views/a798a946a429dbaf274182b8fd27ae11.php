...
<div class="max-w-2xl mx-auto px-4 py-16 text-center">
    <div class="bg-white rounded-2xl shadow-sm p-12">
        ...
        <p class="text-orange-500 font-bold text-lg mb-6"><?php echo e($order->order_number); ?></p>

        <?php if($accountCreated ?? false): ?>
            <div class="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 text-left">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-green-600">Your account has been created with phone number <strong><?php echo e(auth()->user()->phone); ?></strong>.</p>
        ...
        <div class="bg-gray-50 rounded-xl p-6 text-left mb-8">
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <p class="font-semibold capitalize text-gray-800"><?php echo e($order->payment_method === 'cod' ? 'Cash on Delivery' : strtoupper($order->payment_method)); ?></p>
                </div>
                <div>
                    <p class="text-gray-500">Order Total</p>
                    <p class="font-semibold text-gray-800">৳<?php echo e(number_format($order->total)); ?></p>
                </div>
                <div class="col-span-2">
                    <p class="text-gray-500">Shipping to</p>
                    <p class="font-semibold text-gray-800"><?php echo e($order->shipping_name); ?>, <?php echo e($order->shipping_address); ?>, <?php echo e($order->shipping_city); ?></p>
                </div>
            </div>
        </div>

        <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 justify-center">
            <a href="<?php echo e(route('orders.show', $order->id)); ?>" class="bg-orange-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-600 transition">
                View Order Details
            </a>
            <a href="<?php echo e(route('shop.index')); ?>" class="border border-gray-200 text-gray-700 px-6 py-3 rounded-xl font-bold hover:bg-gray-50 transition">
                Continue Shopping
            </a>
        </div>
    </div>
...
<?php if($shouldTrackPurchase ?? false): ?>
<?php
...
(function () {
    var items = <?php echo json_encode($trackedItems, 15, 512) ?>;
    var orderId = <?php echo json_encode($order->order_number, 15, 512) ?>;
    var value = <?php echo e((float) $order->total); ?>;
    var currency = <?php echo json_encode($currencyCode, 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\completeEcom\resources\views/checkout/success.blade.php ENDPATH**/ ?>