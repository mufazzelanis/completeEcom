...
?>
<a href="<?php echo e(route('products.show', $fsp->product->slug)); ?>" class="flex-shrink-0 w-32 sm:w-36 md:w-40 group">
    <div class="relative overflow-hidden bg-gray-50 rounded-lg aspect-square mb-2">
        <?php if($fsp->product->image): ?>
            <img src="<?php echo e(Storage::url($fsp->product->image)); ?>" alt="<?php echo e($fsp->product->name); ?>"
    ...
    <p class="text-sm font-bold text-red-500">৳<?php echo e(number_format($flashPrice)); ?></p>
    <p class="text-[10px] text-gray-400 line-through">৳<?php echo e(number_format($basePrice)); ?></p>
    <div class="mt-1.5 bg-orange-100 rounded-full h-3 overflow-hidden relative">
        <div class="bg-gradient-to-r from-orange-400 to-red-500 h-full rounded-full transition-all" style="width: <?php echo e(max(10, $soldPct)); ?>%"></div>
        <span class="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-700">
            <?php if($fsp->stock_limit > 0 && $soldPct >= 90): ?>
                Almost Gone!
            <?php elseif($fsp->stock_limit > 0): ?>
                <?php echo e($soldPct); ?>% sold
            <?php else: ?>
                Hot
            <?php endif; ?>
        </span>
...
<?php /**PATH C:\xampp\htdocs\azad-ecom\resources\views/partials/flash-sale-card.blade.php ENDPATH**/ ?>