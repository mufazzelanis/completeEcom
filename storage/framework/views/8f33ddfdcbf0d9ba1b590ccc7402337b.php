<?php $__env->startSection('settings-title', 'Security'); ?>

<?php $__env->startSection('settings-content'); ?>
<form method="POST" action="<?php echo e(route('admin.settings.update', 'security')); ?>">
...
<div class="bg-white rounded-xl shadow-sm border p-6 space-y-4">
    ...
    <label class="flex items-center gap-2 cursor-pointer">
        ...
        <input type="checkbox" name="recaptcha_enabled" value="1" class="rounded text-orange-600"
               <?php if(setting('recaptcha_enabled','0') == '1'): echo 'checked'; endif; ?>>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select name="recaptcha_version" class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500">
                <option value="v2" <?php if(setting('recaptcha_version','v2')==='v2'): echo 'selected'; endif; ?>>v2 (Checkbox)</option>
                <option value="v3" <?php if(setting('recaptcha_version','v2')==='v3'): echo 'selected'; endif; ?>>v3 (Invisible)</option>
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Site Key</label>
            <input type="text" name="recaptcha_site_key" value="<?php echo e(setting('recaptcha_site_key', '')); ?>"
        ...
        <div>
            ...
            <input type="password" name="recaptcha_secret_key" value="<?php echo e(setting('recaptcha_secret_key', '')); ?>"
...
<div class="bg-white rounded-xl shadow-sm border p-6 space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" name="login_max_attempts" value="<?php echo e(setting('login_max_attempts', '5')); ?>"
                   min="1" max="20"
                   class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Lockout Duration (minutes)</label>
            <input type="number" name="login_lockout_minutes" value="<?php echo e(setting('login_lockout_minutes', '15')); ?>"
                   min="1"
                   class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500">
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <input type="checkbox" name="two_factor_enabled" value="1" class="rounded text-orange-600"
                   <?php if(setting('two_factor_enabled','1') == '1'): echo 'checked'; endif; ?>>
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <input type="checkbox" name="force_https" value="1" class="rounded text-orange-600"
                   <?php if(setting('force_https','0') == '1'): echo 'checked'; endif; ?>>
...
<div class="bg-white rounded-xl shadow-sm border p-6 space-y-4">
    ...
    <label class="flex items-center gap-2 cursor-pointer">
        ...
        <input type="checkbox" name="ip_restriction_enabled" value="1" class="rounded text-orange-600"
               <?php if(setting('ip_restriction_enabled','0') == '1'): echo 'checked'; endif; ?>>
    ...
    <div>
        ...
        <textarea name="allowed_ips" rows="4"
                  class="w-full border rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-orange-500"
                  placeholder="127.0.0.1&#10;192.168.1.1&#10;203.0.113.0/24"><?php echo e(setting('allowed_ips', '')); ?></textarea>
        <p class="text-xs text-gray-400 mt-1">Your current IP: <?php echo e(request()->ip()); ?></p>
    </div>
</div>
...
<?php echo $__env->make('admin.settings.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\completeEcom\resources\views/admin/settings/security.blade.php ENDPATH**/ ?>