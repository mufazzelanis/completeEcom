...
<div class="bg-white mt-4">
    <div class="max-w-[1200px] mx-auto px-4 py-3">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <?php $__currentLoopData = $highlights; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $f): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if($f['link']): ?>
                    <a href="<?php echo e($f['link']); ?>" <?php if($f['blank']): ?> target="_blank" rel="noopener" <?php endif; ?>
                       class="flex items-center gap-3 -m-1 p-1 rounded-lg hover:bg-orange-50 transition">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-gray-400 text-[10px] truncate"><?php echo e($f['sub']); ?></p>
                        </div>
                    </a>
                <?php else: ?>
...
<div class="bg-white mt-4">
    <div class="max-w-[1200px] mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="<?php echo e(route('categories.index')); ?>" class="text-orange-500 hover:text-orange-700 font-bold text-sm transition">VIEW ALL →</a>
        </div>
...
<?php $__currentLoopData = $homeSections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <?php $sec = $entry['section']; $totalCount = $entry['totalCount']; ?>
    <div class="mt-4 <?php echo e($sec->theme === 'sale' ? 'bg-gradient-to-r from-red-500 to-orange-500' : 'bg-white'); ?>" x-data="{ expanded: false }">
        <div class="max-w-[1200px] mx-auto px-4 py-6">
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-3">
            ...
            <div class="grid <?php echo e($sec->getGridColsClass()); ?> gap-3">
                <?php $__currentLoopData = $entry['products']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($i < $sec->product_limit): ?>
            ...
            <?php if($totalCount > $sec->product_limit): ?>
            <div class="text-center mt-6" x-show="!expanded">
                <button type="button" @click="expanded = true"
                   class="inline-block <?php echo e($sec->theme === 'sale' ? 'bg-white text-orange-600 hover:bg-gray-100' : 'bg-orange-500 text-white hover:bg-orange-600'); ?> px-10 py-2.5 rounded-lg font-bold text-sm transition shadow-md"><?php echo e($sec->getViewAllLabelText()); ?></button>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
<div class="mt-4 py-8 bg-gradient-to-r from-indigo-50 via-white to-orange-50 overflow-hidden">
    ...
    <div class="relative marquee-pause" style="-webkit-mask-image:linear-gradient(to right, transparent, black 5%, black 95%, transparent); mask-image:linear-gradient(to right, transparent, black 5%, black 95%, transparent);">
        <div class="flex gap-4 w-max animate-marquee">
            <?php $__currentLoopData = $testimonials->concat($testimonials); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $review): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php $theme = $reviewThemes[$loop->index % count($reviewThemes)]; ?>
                <div class="flex-shrink-0 w-72 bg-gradient-to-br <?php echo e($theme['bg']); ?> rounded-2xl shadow-md ring-1 <?php echo e($theme['ring']); ?> overflow-hidden relative">
                    <div class="h-1.5 bg-gradient-to-r <?php echo e($theme['bar']); ?>"></div>
                    <div class="p-5 relative">
                        <svg class="w-9 h-9 <?php echo e($theme['quote']); ?> absolute top-3 right-4" fill="currentColor" viewBox="0 0 24 24"><path d="M7.17 6A5.17 5.17 0 002 11.17v6.66A2.17 2.17 0 004.17 20h4.66A2.17 2.17 0 0011 17.83v-4.66A2.17 2.17 0 008.83 11H5a3.17 3.17 0 013.17-3.17V6H7.17zm11 0A5.17 5.17 0 0013 11.17v6.66A2.17 2.17 0 0015.17 20h4.66A2.17 2.17 0 0022 17.83v-4.66A2.17 2.17 0 0019.83 11H16a3.17 3.17 0 013.17-3.17V6h-1z"/></svg>
                        <div class="flex mb-2">
                            <?php for($i = 1; $i <= 5; $i++): ?>
                                <svg class="w-4 h-4 <?php echo e($i <= $review->rating ? $theme['star'] : 'text-white/60'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.958a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.367 2.447a1 1 0 00-.363 1.118l1.287 3.957c.3.922-.755 1.688-1.539 1.118l-3.367-2.446a1 1 0 00-1.176 0l-3.367 2.446c-.783.57-1.838-.196-1.539-1.118l1.286-3.957a1 1 0 00-.363-1.118L2.373 9.385c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.958z"/></svg>
                            <?php endfor; ?>
                        </div>
                        <p class="text-gray-700 text-sm leading-relaxed line-clamp-4 mb-4 min-h-[4.5rem] font-medium">&ldquo;<?php echo e($review->comment); ?>&rdquo;</p>
                        <div class="flex items-center gap-3 pt-3 border-t border-white/60">
                            <div class="w-9 h-9 bg-gradient-to-br <?php echo e($theme['avatar']); ?> rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
                                <span class="text-white font-bold text-xs"><?php echo e(strtoupper(substr($review->user->name, 0, 1))); ?></span>
                            </div>
                            <div class="min-w-0">
                                <p class="font-semibold text-gray-800 text-sm truncate"><?php echo e($review->user->name); ?></p>
                                <?php if($review->product): ?>
                                    <p class="text-gray-500 text-[11px] truncate">on <a href="<?php echo e(route('products.show', $review->product->slug)); ?>" class="hover:text-orange-600 hover:underline transition"><?php echo e($review->product->name); ?></a></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
...
<div class="bg-white mt-4">
    <div class="max-w-[1200px] mx-auto px-4 py-6">
        <div class="flex items-center justify-center mb-5">
            ...
            <h2 class="text-lg font-extrabold text-gray-900 px-6"><?php echo e(setting('just_for_you_title', 'Just For You')); ?></h2>
            <div class="h-px bg-gray-200 flex-1"></div>
        </div>
        ...
        <div class="text-center mt-6">
            <a href="<?php echo e(route('shop.index')); ?>" class="inline-block bg-orange-500 text-white px-10 py-2.5 rounded-lg font-bold text-sm hover:bg-orange-600 transition shadow-md"><?php echo e(setting('just_for_you_button_text', 'View More Products')); ?></a>
        </div>
    </div>
</div>
<?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\azad-ecom\resources\views/home.blade.php ENDPATH**/ ?>